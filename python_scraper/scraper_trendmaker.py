...
def parse_price(price_str):
    ...
    # If we have format like "17.999" (Serbian thousands separator)
    if '.' in clean:
        parts = clean.split('.')
...
def get_product_code(prod):
    # First try product code from h5.product-code (Trendmaker specific)
...
def scrape_trendmaker():
    all_codes = set()
    batch = []
    total_count = 0

    for category_url in CATEGORY_URLS:
        print(f"[Trendmaker] Processing category {category_url}")
        ...
        for idx, url in enumerate(page_urls, 1):
            ...
            for prod in products:
                ...
                brand_elem = prod.select_one("[itemprop='brand']") or prod.select_one("meta[itemprop='brand']")
                
                # Trendmaker specific price selectors
                ...
                # Brand is always Adidas for Trendmaker (hardcoded)
                brand_val = "Adidas"
                ...
                if len(batch) >= BATCH_SIZE:
                    ...
                    print(f'[OK] Trendmaker batch: {len(batch)} products sent, API response: {response.status_code}')
                    batch = []
    ...
    if batch:
        ...
        print(f'[OK] Trendmaker final batch: {len(batch)} products sent, API response: {response.status_code}')

    print(f"[Trendmaker] COMPLETED: Total {total_count} products processed")
